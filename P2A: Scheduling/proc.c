...
struct proc *hold_lottery(int total_tickets) {
    ...
    uint winner_ticket_number = rand() % total_tickets;    

    // pick the winning process from ptable.
    struct proc *p; struct proc *winner;
    int count = 0; int ifFound = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if (p->state == RUNNABLE) {
        ...
        if (ifFound == 0 && count > winner_ticket_number) {
          ifFound = 1;
          winner = p;
        }
      }
...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  // np->runTicks = 0;
  // np->boostsleft = 0;
  // To do: Initialize runTicks and bootesLeft;
...
wakeup1(void *chan)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == SLEEPING ) {
      ...
      //if (p->sleepTicks > 0) p->sleepTicks -= 1;
      if (p->chan == chan && p->sleepTicksCount >= p->sleepTicks) {
        p->state = RUNNABLE;
        p->sleepTicksCount = 0;
        p->sleepTicks = 0;
      }
