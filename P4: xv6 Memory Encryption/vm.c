...
void
insertQueue(uint vpn, pte_t* pte){
...
void
removeQueue(uint vpn)
{
    struct proc *currproc = myproc();
    int startIndex = currproc->clockIndex;
    int matchedIndex = -1;

    // Find the node in the queue that will be removed.
    ...
    if (matchedIndex == -1) return;
		...
		mencrypt((char*)currproc->clockQueue[matchedIndex].vpn, 1);
		
    for (int i = matchedIndex; i != startIndex; i = (i + 1) % CLOCKSIZE) {
        int nextIndex = (i + 1) % CLOCKSIZE;
        currproc->clockQueue[i].pte = currproc->clockQueue[nextIndex].pte;
        currproc->clockQueue[i].vpn = currproc->clockQueue[nextIndex].vpn;
    }

    currproc->clockQueue[startIndex].vpn = -1;
...
int mdecrypt(char *virtual_addr) {
  ...
  // Check if kernel address isvalid.
  if(kernelAddr == 0){
    return -1;
  }
  ...
  if(!((*pte & PTE_E) == 0) && ((*pte & PTE_P) == 0)){ 
    // Decrypt all bits within that page
    ...
    *pte = (*pte) & (~PTE_E);
    ...
    insertQueue((uint) alignedAddr, pte);
    //Success
    return 0; 
  }
  return -1;
...
int mencrypt(char *virtual_addr, int len) {
  ...
  for(int i = 0; i < len; i++){
    pageStart = virtual_addr + (i * PGSIZE);
    kernelAddr = uva2ka(pd, pageStart);
    if(kernelAddr == 0) continue;
    
    pte = walkpgdir(pd, pageStart, 0);

		if (*pte & PTE_E ) continue;
    ...
    *pte = (*pte) | PTE_E;
    ...
    // Encrypt the bit within that page.
...
int getpgtable(struct pt_entry* pt_entries, int num, int wsetOnly) {  
  if (pt_entries == 0 || (wsetOnly != 0 && wsetOnly != 1)) return -1;
  ...
  int numPagesSearched = 0;
	int numPagesFilled = 0;
  // only fill upto num pages, if there are less valid pages than num, exit after we've searched all pages
	while (numPagesFilled < num && numPagesSearched < totalPages) {
    pte = walkpgdir(currProc->pgdir, currAddr, 0);
		if (wsetOnly == 0) {
			if ((*pte & PTE_E) != 0 || (*pte & PTE_P) != 0) {
				pt_entries[numPagesFilled].pdx = PDX(currAddr);
        pt_entries[numPagesFilled].ptx = PTX(currAddr);
        pt_entries[numPagesFilled].ppage = *pte >> PTXSHIFT;
        pt_entries[numPagesFilled].present = (*pte & PTE_P) ? 1 : 0;
        pt_entries[numPagesFilled].writable = (*pte & PTE_W) ? 1 : 0;
        pt_entries[numPagesFilled].user = (*pte & PTE_U) ? 1 : 0;
        pt_entries[numPagesFilled].encrypted = (*pte & PTE_E) ? 1 : 0;
        pt_entries[numPagesFilled].ref = (*pte & PTE_A) ? 1 : 0;
        numPagesFilled++;
			}
		} else {
			if (((*pte & PTE_E) == 0) && ((*pte & PTE_P) != 0)) {
